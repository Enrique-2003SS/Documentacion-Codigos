...
Nos dimos cuenta de que algunas columnas están unidas, o al menos así parecen. Pero eso no nos conviene, así que las vamos a separar. También vamos a renombrar las dos primeras columnas
...
La función `gsub` busca la palabra `"Hidalgo"` y la remplaza por `""`, en otras palabras la elimina por completo, sin dejar ningún espacio. Después usamos la función `str_trim` de la librería `stringr` para quitar los espacios que estén al principio y al final del texto. Esto es útil porque, cuando juntamos palabras o textos usando `paste` o `paste0`, esos espacios pueden causar problemas, ademas cuando agrupamos tambien pueden existir dicho problema.

Ahora vamos a crear una nueva columna con el nombre de `ent` el cual indicaremos en cada fila que es `Hidalgo`.
